...
export const configColumns = (
  ...
  return [
    ...
    },
    {
      ...
      key: 'summary',
      width: '30%',
      render: (text: string) => <span>{text.length > 100 ? `${text.slice(0, 100)}...` : text}</span>,
    },
    {
      title: 'Tác giả',
      dataIndex: 'author',
      key: 'author',
      width: '10%',
      render: (_, origin) => <div>{origin.author?.name}</div>,
    },
    {
      title: 'Trạng thái',
      dataIndex: 'status',
    ...
    },
    {
      ...
      render: (_, original) => (
        ...
        >
          <Button style={{ padding: '2px 6px', border: 'none' }} onClick={() => handleClickEdit(original)}>
            <EditOutlined />
          </Button>
          <Button style={{ padding: '2px 6px', border: 'none' }} onClick={() => showDeleteConfirm(original)}>
            <DeleteOutlined />
          </Button>
          <Button
