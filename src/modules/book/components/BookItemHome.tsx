      ...
      <Link className="text-white" to={`/book/${bookItem.id}`}>
        <div className="relative w-full bg-[#f5f5f5] max-[767px]:hidden">
          <div
            ...
            style={{ backgroundImage: `url(${bookItem.thumbnail_url})`, backgroundPosition: 'center 80%' }}
          ></div>
          {/* Flag */}
          <div></div>
          {/* Butto read */}
          <div className="xxxl:size-10 absolute bottom-2 right-3 flex size-8 text-primary hover:text-[#ff5b9a]">
            <ButtonRead />
          </div>
        </div>
      </Link>
        ...
        <div className="relative flex h-[258px] w-full px-2 max-[1679px]:h-[220px] max-[767px]:hidden">
          <div className="w-full">
            ...
            <div className="flex flex-col">
              ...
              <div className="xxxl:mt-2 xxxl:h-6 mt-[6px] h-[22px] overflow-hidden">
                {bookItem.category_book.map((category) => (
                  <Tag
                    key={category.category_id}
                    bordered={false}
                    className="xxxl:text-[13px] mb-[2px] bg-[rgba(0,0,0,0.08)] text-[11px]"
                  >
                    {category.category_name}
                  </Tag>
                ))}
              </div>
              <section className="xxxl:mt-2 xxxl:h-[118px] mt-[6px] h-[100px]">
                <p className="xxxl:text-[14px] line-clamp-5 text-[12px]">{bookItem.summary}</p>
              </section>
              <div className="xxxl:bottom-[12px] absolute bottom-[10px] right-0 flex w-full items-center justify-end px-2 text-primary hover:text-primary/75">
                <span className="xxxl:text-[14px] text-[12px]">Xem thêm</span>
          ...
          <Link to={`/book/${bookItem.id}`} className="text-white">
            <div className="relative z-[2] overflow-hidden rounded-md before:block before:pt-[146.25%]">
              ...
              >
                ...
                <div className="xxxl:size-8 absolute bottom-[10px] right-2 flex size-6 text-[rgba(0,0,0,0.5)] transition-all group-hover:text-primary">
                  <ButtonRead />
                </div>
              </div>
            </div>
