...
interface BookItemProps {
  bookItem: RecommendedBook
}

export default function RecommendedBookItem({ bookItem }: BookItemProps) {
  ...
  const content = (
    <div className="w-full">
      <Link className="text-white" to={`/book/${bookItem.book_id}`}>
        <div className="relative w-full bg-[#f5f5f5] max-[767px]:hidden">
          <div
            className="w-full bg-[rgb(182,182,182)] bg-cover bg-no-repeat pt-[56%]"
            style={{
      ...
      <div className="cursor-pointer select-none" onClick={() => navigate(`/book/${bookItem.book_id}`)}>
    ...
    <>
      ...
      >
        <div ref={triggerRef} className="group relative max-[767px]:hover:scale-[1.05]" style={{ transition: '0.3s' }}>
          <Link to={`/book/${bookItem.book_id}`} className="text-white">
          ...
          <div
            ...
            onClick={() => navigate(`/book/${bookItem.book_id}`)}
          >
